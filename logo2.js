function setup(){ //opens dashboard with payper logo animation
  ...
  function draw() {
    ...
    context.save();
    context.rotate(-Math.PI/2);
    drawQuadrant("#39D7A7",edge,(scale > 1 ? 1 : scale));
    context.save();
    context.rotate(-Math.PI/2);
    context.scale(1,-1);
    context.translate(0,-edge);
    drawQuadrant("#47D9B5",edge,(scale > 2 ? 1 : scale-1));
    context.restore();
    context.translate(edge,-edge);
    drawQuadrant("#50DCC2",edge,(scale > 3 ? 1 : scale-2));
    context.restore();
    context.rotate(Math.PI/2);
    context.scale(1,-1);
    ...
    context.rotate(Math.PI/2);
    context.scale(1,-1);
    drawQuadrant("#0ECE95",edge,(scale > 3 ? 1 : scale-2));
